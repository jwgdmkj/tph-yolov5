...
@torch.no_grad()
def run(weights=ROOT / './weights/yolov5l-xs-1.pt',  # model.pt path(s)
        source=ROOT / '/data/dataset/drone_veiw_dataset/UAV-benchmark-S/UAV-benchmark-S/',  # file/dir/URL/glob, 0 for webcam
    ...
    origin_save_dir = increment_path(Path(project) / name, exist_ok=exist_ok)  # increment run
    origin_source = source

    for sc in sourcedir:
        source = origin_source + sc
        save_dir = origin_save_dir / sc
        print(save_dir)
